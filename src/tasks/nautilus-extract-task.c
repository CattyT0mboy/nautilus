...
static gpointer
object_list_copy_func (gconstpointer src,
                       gpointer      data)
{
                       ...
                       gpointer         user_data)
{
    ...
    if (nautilus_task_is_canceled (NAUTILUS_TASK (task)))
    {
...
execute (NautilusTask *task)
{
    ...
    for (l = self->source_files, i = 0;
         l != NULL && !nautilus_task_is_canceled (task);
         l = l->next, i++)
    {
        GFile *source_file;
    ...
    if (!nautilus_task_is_canceled (task))
    {
